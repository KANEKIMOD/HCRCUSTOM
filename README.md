...
```bash
curl -fsSL https://raw.githubusercontent.com/KANEKIMOD/HCRCUSTOM/main/setup.sh | bash --transport plain --port 8880 --no-menu
```

TLS:

```bash
curl -fsSL https://raw.githubusercontent.com/KANEKIMOD/HCRCUSTOM/main/setup.sh | bash --transport tls --port 8880 --no-menu
```

Auto:

```bash
curl -fsSL https://raw.githubusercontent.com/KANEKIMOD/HCRCUSTOM/main/setup.sh | bash --transport auto --port 8880 --no-menu
```
...
El instalador detiene `hcr-server` antes de reemplazar `hcr-server`, evitando el error `Text file busy`.
